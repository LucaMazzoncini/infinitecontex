...
Create and verify an exact profile with:

    infctx model profile create qwen3.6:35b --expected-digest DIGEST
    infctx model profile verify PROFILE_ID

For each explicit agent run call, packing and admission records are written before Ollama is contacted. Inspect compact call linkage with infctx plan agent-step evidence PLAN_ID RUN_ID --json. Sampling options sent to Ollama include the exact output and context limits. Candidate creation still stops before approval, application, or validation.